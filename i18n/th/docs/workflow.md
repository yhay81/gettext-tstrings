...
ทั้งหมดนี้สรุปได้เป็นการตรวจหกอย่าง จึงขอวางไว้ตรงนี้ก่อน แต่ละหัวข้อด้านล่างจัดวางหนึ่งในนั้นให้
...
มีไฟล์หนึ่งที่มีบทบาทในแต่ละทิศทาง: `.pot` พาข้อความของคุณ *ออกไป* หานักแปล ส่วนไฟล์ `.po` พาคำแปล *กลับมา* ส่วนที่เหลือของหน้านี้คือสิ่งที่เคลื่อนที่อยู่ระหว่างสองสิ่งนั้น
...
`pybabel init` ในบทแนะนำโดยปกติรันเพียงครั้งเดียว ตอนที่เพิ่มภาษาใหม่ นับจากนั้นวงจรการทำงานคือ **extract → update → translate → compile** และศูนย์กลางของมันคือ `pybabel update` ซึ่งพับเทมเพลตชุดใหม่เข้ากับแคตตาล็อกที่มีอยู่โดยไม่ทิ้งคำแปลที่อยู่ในนั้นอยู่แล้ว
...
Babel สังเกตว่า msgid ใหม่คล้ายกับอันที่ถูกลบไป จึงจับคู่มันกับคำแปลเดิม — แต่ติดธง **fuzzy** ให้กับคู่นั้น: การเดาของเครื่องที่รอมนุษย์มายืนยัน ธงนี้เปลี่ยนสิ่งที่ถูกคอมไพล์ `pybabel compile` **ไม่รวมรายการ fuzzy ลงใน `.mo`** ดังนั้นจนกว่านักแปลจะยืนยันคู่นี้ แอปพลิเคชันจะเรนเดอร์ข้อความภาษาอังกฤษตัวใหม่ ไม่ใช่ภาษาญี่ปุ่นที่ตกยุคไปแล้ว:
...
!!! bug "Babel 2.18.0: `--check` กั้นแคตตาล็อกที่ใช้บริบทไม่ได้"

    บน Babel 2.18.0 `pybabel update --check` จะรายงานว่าแคตตาล็อกที่มี `msgctxt` **ทุกอัน** ล้าสมัย ทุกครั้งที่รัน ไม่ว่ามันจะทันสมัยเพียงใดก็ตาม ด่านที่ล้มเหลวตลอดเวลาแย่กว่าการไม่มีด่านเลย เพราะทีมจะปิดมันทิ้ง — ดังนั้นถ้าคุณใช้ `pgettext` หรือ `npgettext` แม้เพียงเล็กน้อย ก็จงแทนที่ขั้นตอนนี้แทนที่จะทนอยู่กับมัน การอ่านเทมเพลตและแคตตาล็อกแต่ละอันด้วย `babel.messages.pofile.read_po` แล้วเปรียบเทียบ `{(m.context, m.id) for m in catalog if m.id}` คือทั้งหมดของการตรวจนี้ และเป็นสิ่งที่[การ build ของเว็บไซต์นี้เอง](index.md)ทำอยู่ ส่วนสาเหตุนั้นมี[บันทึกไว้ในหน้าหลุมพราง](pitfalls.md#your-tools-have-bugs-too)
