...
| ชื่อ | จุดประสงค์ |
| --- | --- |
| `lazy_gettext(template, /, *, strict=False)` | เลื่อนการแปลออกไปจนถึงการเรนเดอร์แต่ละครั้ง |
| `lazy_pgettext(context, template, /, *, strict=False)` | รูปแบบที่มีคอนเท็กซ์ |
| `LazyString` | สิ่งที่ทั้งสองฟังก์ชันคืนค่า เรนเดอร์ผ่าน `str()` และ `format()` ในภาษาที่ผูกอยู่ ณ ขณะนั้น เปรียบเทียบเท่ากับข้อความที่เรนเดอร์ออกมา และแฮชไม่ได้โดยตั้งใจ |

ตัวอย่างการใช้งานจริง รวมถึงเหตุผลที่ `strict` ควรอยู่ที่จุดนิยาม อยู่ใน
[การแปลแบบเลื่อนเวลา](guide.md#deferred-translation)
