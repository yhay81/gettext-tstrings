...
| Ім'я | Призначення |
| --- | --- |
| `lazy_gettext(template, /, *, strict=False)` | Відкласти переклад до кожного рендерингу. |
| `lazy_pgettext(context, template, /, *, strict=False)` | Контекстна форма. |
| `LazyString` | Те, що повертають обидві. Рендериться через `str()` і `format()` тією мовою, яка прив'язана в цей момент, дорівнює своєму відрендереному тексту при порівнянні й навмисно негешований. |

Розібрані приклади, зокрема чому `strict` належить до місця означення, — у
розділі [Відкладений переклад](guide.md#deferred-translation).
