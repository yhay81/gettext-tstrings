...
!!! warning "Робочий потік стартує без прив'язки"

    Голий `threading.Thread` чи `ThreadPoolExecutor.submit` починає з чистого
    контексту й не успадковує прив'язку — виклик відкочується до глобального
    для процесу каталогу gettext. Переносьте контекст явно:
