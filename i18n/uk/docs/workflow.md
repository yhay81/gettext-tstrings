...
*назовні* до перекладачів, файли `.po` везуть переклади *назад*. Усе нижче —
рух між цими двома.
...
`pybabel init` із підручника запускається один раз на мову — назавжди. Далі
робочий цикл — **видобути → оновити → перекласти → скомпілювати**, і його
центр — `pybabel update`, який вливає свіжий шаблон у наявні каталоги, не
викидаючи перекладів, що вже в них є.
...
Прапорець має зуби. `pybabel compile` **виключає записи fuzzy з `.mo`**, тож
...
!!! bug "`--check` не може вартувати каталог, що використовує контексти"
    ...
    свіжим той був. Порівняння йде через `Catalog.is_identical`, який шукає
    кожне повідомлення за ключем, під яким воно збережене, — а для контекстного
    повідомлення цей ключ є парою `(id, context)`, якої `Catalog.get` не
    приймає. Пошук не повертає нічого, і каталоги ніколи не виявляються
    рівними:
    ...
    Тож якщо ви взагалі користуєтесь `pgettext` чи `npgettext` — а розрізнення
    омонімів і є причиною їхнього існування, — цей крок фактично пропускає
    все, і то в найгірший спосіб: завжди червоний, тож команда його вимикає, і
    застарілості не вартує вже ніщо. Доки це не виправлять в апстрімі,
    порівнюйте набори повідомлень самотужки. Прочитати шаблон і кожен каталог
    через `babel.messages.pofile.read_po` й порівняти
    `{(m.context, m.id) for m in catalog if m.id}` — це і є вся перевірка, і
    саме це робить [власна збірка цього сайту](index.md).
