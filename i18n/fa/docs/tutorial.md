...
همان کد محلی تنها جایی است که این زبان را نام می‌برد.
...
مترجم‌ها معمولاً به‌جای کد مبدأ از کاتالوگ‌ها کار می‌کنند، پس فایل
کوچکی به نام **کاتالوگ** میان شما و آن‌ها رفت‌وآمد می‌کند. نخستین گام
برای ساختن آن، گرد آوردن همهٔ پیام‌های علامت‌گذاری‌شده از دل کد است.
...
یک نکتهٔ درخور دانستن از همین حالا: خطا را گزارش می‌کند و با کد ناصفر
بیرون می‌آید، اما `.mo` را به هر حال می‌نویسد. در یک پروژهٔ واقعی این CI
است که باید بر آن کد خروج بایستد — [در محیط
عملیاتی](workflow.md#what-ci-gates) همین را برپا می‌کند.

## ۵. اجرا { #5-run-it }

گام‌های ۲ تا ۴ از `tr()` استفاده می‌کردند که دنبال کاتالوگ می‌گردد و
هیچ نمی‌یابد. حالا که کاتالوگی هست، آن را بار کنید و یک‌بار مقید کنید:
`Translator` کاتالوگ را نگه می‌دارد تا محل‌های فراخوانی مجبور نباشند
...
1. کتابخانهٔ استاندارد `.mo`ِ کامپایل‌شده را بار می‌کند و `Translator`
   ...
   همان ترجمه‌ای را انجام می‌دهد که `tr` می‌کرد، بسته‌شده به یک کاتالوگ.
...
- [استخراج](extraction.md) — مرجع کامل `pybabel`: نام‌های تابع سفارشی،
  ...
  می‌کنند.
- [مهاجرت](migration.md) — اگر پروژه‌ای که واقعاً می‌خواهید این کار را
  در آن بکنید، از پیش کاتالوگ‌های gettext دارد.
- [برای مترجمان](translators.md) — تنها صفحه‌ای که باید به دست کسی
  بدهید که آن خط‌های `msgstr` را پر می‌کند.
