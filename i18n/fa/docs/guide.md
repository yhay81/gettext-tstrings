...
!!! warning "یک نخ کارگر بدون بسته آغاز می‌شود"

    یک `threading.Thread` ساده، یا `ThreadPoolExecutor.submit`، با
    بافتاری تازه آغاز می‌شود و بسته را به ارث نمی‌برد — فراخوانی به
    کاتالوگ سراسریِ gettext در سطح فرایند بازمی‌گردد. بافتار را صریحاً
    با خود ببرید:
