...
переводчикам, `.po`-файлы несут переводы *обратно*. Всё, что ниже, —
...
`pybabel init` из учебника выполняется один раз на язык — и больше никогда.
Дальше рабочий цикл — **извлечь → обновить → перевести → скомпилировать**, и
...
человека. У флага есть зубы. `pybabel compile` **исключает fuzzy-записи из
`.mo`**, поэтому, пока переводчик не подтвердит пару, приложение выводит
...
!!! bug "`--check` не может проверять каталог, использующий контексты"
    ...
    свежим тот ни был. Сравнение идёт через `Catalog.is_identical`, который
    ищет каждое сообщение по ключу, под которым оно хранится, — а для
    контекстного сообщения этот ключ представляет собой пару `(id, context)`,
    которую `Catalog.get` не принимает. Поиск ничего не возвращает, и каталоги
    никогда не оказываются равными:
    ...
    Поэтому, если вы вообще пользуетесь `pgettext` или `npgettext` — а
    различение слов-омонимов и есть причина их существования, — этот шаг
    отказывает наихудшим из возможных способов: он всегда красный, поэтому
    команда его отключает, поэтому устаревание не проверяет уже ничто. Пока
    это не исправлено в самом Babel, сравнивайте наборы сообщений сами.
    Прочитать шаблон и каждый каталог через `babel.messages.pofile.read_po` и
    сравнить `{(m.context, m.id) for m in catalog if m.id}` — вот и вся
    проверка, и именно её делает
    [собственная сборка этого сайта](index.md).
