...
| Имя | Назначение |
| --- | --- |
| `lazy_gettext(template, /, *, strict=False)` | Откладывает перевод до каждого рендеринга. |
| `lazy_pgettext(context, template, /, *, strict=False)` | Вариант с контекстом. |
| `LazyString` | То, что возвращают оба. Рендерится через `str()` и `format()` на том языке, который привязан в этот момент, сравнивается со своим отрендеренным текстом и намеренно не хешируется. |

Разобранные примеры, в том числе почему `strict` место при определении, — в
разделе [Отложенный перевод](guide.md#deferred-translation).
