...
Направьте `app.py` на скомпилированный каталог:
...
`_` — традиционное в gettext имя для «переведи это», короткое, потому что оно
стоит у каждой видимой пользователю строки. Это та же функция, что и `tr`,
привязанная к одному каталогу.
...
Вот и весь цикл: **пометить → извлечь → перевести → скомпилировать →
запустить**. Всё остальное на этом сайте — уточнение одного из этих пяти
шагов.
