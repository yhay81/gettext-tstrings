...
Переводчики обычно работают с каталогами, а не с исходным кодом, поэтому между
вами и ними путешествует небольшой файл — **каталог**. Первый шаг к нему —
собрать из кода все помеченные сообщения.
...
[В продакшене](workflow.md#what-ci-gates) это настраивает.
...
1. Стандартная библиотека загружает скомпилированный `.mo`, а `Translator`
   ...
   пользователю строки. Он выполняет тот же перевод, что и `tr`, привязанный
   к одному каталогу.
...
- [Миграция](migration.md) — если в проекте, где вы на самом деле хотите всё
  это применить, уже есть каталоги gettext.
- [Переводчикам](translators.md) — одна страница, которую можно вручить тому,
  кто заполняет строки `msgstr`.
