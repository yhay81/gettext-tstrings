---
description: "API времени выполнения: какую точку входа выбрать, привязка каталога, язык запроса, отложенные строки, значения с учётом локали и обработка неверных переводов."
---
...
## Какую точку входа выбрать? { #which-entry-point-should-i-use }

Пакет экспортирует несколько способов перевести сообщение, потому что
приложения привязывают язык несколькими разными способами. Выбирайте по тому,
как ваша программа решает, на каком она языке:
...
!!! warning "Наследует ли рабочий поток привязку, зависит от сборки"
...
## Значения с учётом локали { #locale-aware-values }
...
время и единицы измерения — вотчина [Babel][babel-numbers]. Сначала
...
В сообщении со счётом число выполняет две работы — выбирает множественную
форму и появляется в тексте, — и локализуется только вторая. Оставьте сырое
число для выбора формы, а для показа передайте отформатированную строку:
...
Форматирование до вызова — это ещё и то, что удерживает спецификацию формата
вне каталога: переводчик видит готовый кусок текста, а не число с инструкцией,
как его отрисовать.
...
на странице
[Переводчикам](translators.md#reading-a-failure-message). Эта страница написана
