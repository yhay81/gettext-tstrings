...
αλλά τίποτα δεν εξαρτάται από αυτή την επιλογή. Για να χρησιμοποιήσετε άλλη
γλώσσα, αντικαταστήστε το `ja` στο βήμα 4 — αυτός ο κωδικός τοπικών ρυθμίσεων
είναι το μόνο σημείο που την κατονομάζει.
...
Οι μεταφραστές δουλεύουν συνήθως με καταλόγους και όχι με τον πηγαίο κώδικα,
οπότε ένα μικρό αρχείο που λέγεται **κατάλογος** ταξιδεύει ανάμεσα σε εσάς και
σε αυτούς. Το πρώτο βήμα προς αυτόν είναι να συλλεχθεί από τον κώδικα κάθε
επισημασμένο μήνυμα.
...
Μια επιφύλαξη που αξίζει να ξέρετε από τώρα: αναφέρει το σφάλμα και τερματίζει
με μη μηδενικό κωδικό, αλλά γράφει το `.mo` έτσι κι αλλιώς. Σε ένα πραγματικό
έργο, το CI είναι αυτό που πρέπει να σταματήσει σε αυτόν τον κωδικό εξόδου — το
[Στην παραγωγή](workflow.md#what-ci-gates) το ρυθμίζει.

## 5. Εκτελέστε το { #5-run-it }

Τα βήματα 2–4 χρησιμοποίησαν την `tr()`, η οποία ψάχνει για κατάλογο και δεν
βρίσκει κανέναν. Τώρα που υπάρχει ένας, φορτώστε τον και δέστε τον μία φορά: ο
`Translator` κρατά έναν κατάλογο ώστε να μη χρειάζεται να τον κατονομάζουν τα
σημεία κλήσης, και το `_` είναι το καθιερωμένο όνομα του gettext για το
αποτέλεσμα.
...
1. Η τυπική βιβλιοθήκη φορτώνει το μεταγλωττισμένο `.mo` και ο `Translator`
   ...
   συμβολοσειρά που βλέπει ο χρήστης. Εκτελεί την ίδια μετάφραση με την `tr`,
...
- [Μετάβαση](migration.md) — αν το έργο στο οποίο θέλετε πραγματικά να τα
  κάνετε αυτά έχει ήδη καταλόγους gettext.
- [Για μεταφραστές](translators.md) — η μία σελίδα που δίνετε σε όποιον
  συμπληρώνει εκείνες τις γραμμές `msgstr`.
