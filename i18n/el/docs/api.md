...
| Όνομα | Σκοπός |
| --- | --- |
| `lazy_gettext(template, /, *, strict=False)` | Αναβάλλει τη μετάφραση ώς κάθε απόδοση. |
| `lazy_pgettext(context, template, /, *, strict=False)` | Η μορφή με συγκείμενο. |
| `LazyString` | Αυτό που επιστρέφουν και οι δύο. Αποδίδεται μέσω `str()` και `format()` σε όποια γλώσσα είναι δεμένη εκείνη τη στιγμή, συγκρίνεται ίση με το αποδιδόμενο κείμενό της, και είναι σκόπιμα μη κατακερματίσιμη. |

Δουλεμένα παραδείγματα, μαζί με το γιατί το `strict` ανήκει στον ορισμό,
υπάρχουν στην [Αναβαλλόμενη μετάφραση](guide.md#deferred-translation).
