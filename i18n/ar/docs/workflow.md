...
وتحمل ملفات `.po` الترجمات *عائدةً*. كل ما يلي هو حركة المرور بين هذين
الاثنين.
...
يُنفَّذ أمر `pybabel init` من الدرس التعليمي مرة واحدة لكل لغة، وإلى الأبد.
...
لكنه وسم الزوج بعلامة **fuzzy**: تخمينُ آلةٍ ينتظر إنساناً. ولهذه العلامة
أنياب: فأمر `pybabel compile` **يستبعد إدخالات fuzzy من ملف `.mo`**، فإلى
...
!!! bug "`--check` لا يستطيع حراسة كتالوج يستخدم السياقات"

    في Babel 2.18.0، يبلّغ `pybabel update --check` عن **كل** كتالوج يحتوي
    على `msgctxt` بأنه متأخر، في كل تشغيل، مهما كان محدَّثاً. فالمقارنة تجري
    عبر `Catalog.is_identical`، الذي يبحث عن كل رسالة بالمفتاح المخزَّنة
    تحته — ومفتاح الرسالة ذات السياق هو الزوج `(id, context)`، وهو ما لا
    يقبله `Catalog.get`. فيعود البحث بلا شيء، ولا يتساوى الكتالوجان في
    المقارنة أبداً:
    ...
    لذا إن كنت تستخدم `pgettext` أو `npgettext` أصلاً — وتمييز الألفاظ
    المشتركة هو سبب وجودهما — فإن هذه الخطوة تُخفق على أسوأ نحو ممكن: حمراء
    دائماً، فيوقفها الفريق، فلا يحرس شيءٌ تخلُّفَ الكتالوجات. وإلى أن
    يُصلَح ذلك في المنبع، قارن مجموعات الرسائل بنفسك. فقراءة القالب وكل
    كتالوج بـ`babel.messages.pofile.read_po` ومقارنة
    `{(m.context, m.id) for m in catalog if m.id}` هي الفحص كله، وهو ما
    يفعله [بناء هذا الموقع نفسه](index.md).
