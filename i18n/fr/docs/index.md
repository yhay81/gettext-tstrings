...
un autre, ni ajouter son propre formatage.

## Le problème résolu

Une f-string est déjà interpolée lorsqu'une bibliothèque la reçoit : la traduire
revient donc à traduire un fragment. Une t-string ([PEP 750]) conserve
séparément le texte statique, les valeurs évaluées, les expressions source, les
conversions et les spécifications de format. C'est exactement la séparation
qu'attend un catalogue. Découvrez [ce que cela change](comparison.md) par
rapport à `%(name)s` et `.format()`.
...
- **[Pourquoi les t-strings](comparison.md)** — le même message écrit de trois
  façons et ce que chacune confie au catalogue.
- **[Guide](guide.md)** — API d'exécution, langue par requête, chaînes différées
  et gestion d'un catalogue incorrect.
- **[Extraction](extraction.md)** — workflow `pybabel`, configuration et
  validation par les outils existants.
...
- **[API](api.md)** — tout ce que le paquet exporte.
