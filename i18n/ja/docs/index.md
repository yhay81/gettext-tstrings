...
# 翻訳するのはメッセージ全体。<br>文字列の断片ではなく。

`gettext-tstrings`は、Python 3.14以降のt-stringを標準のgettextカタログとBabel
のツール群へつなぎます。値と書式指定はアプリケーションのコードに残り、カタログ
には`{name}`という単純なプレースホルダーを含む完全なメッセージが入ります。
...
alpha版 · Python 3.14以降 · 一般的なPO/MOカタログ · 実行時の依存なし
{ .home-facts }
...
カタログには完全なメッセージ`Hello {name}`が渡されます。翻訳では`{name}`の
順序を変えたり繰り返したりでき、その周りの語はすべて書き換えられます。ただし、
プレースホルダーを省略すること、新しいものをでっち上げること、そこからあなたの
オブジェクトへ手を伸ばすこと、独自の書式指定を付けることはできません。

約束はこれがすべてです。**翻訳は、自分が翻訳するメッセージの構造を変えられない。**
ライブラリはそれを入口で — カタログのコンパイル時に — 検査し、レンダリング時に
もう一度検査します。それでも本番へ届いてしまった不正なエントリは、クラッシュ
せずに警告をログへ記録し、ソースメッセージをレンダリングします。
