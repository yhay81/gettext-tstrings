...
!!! warning "`-c` אינו אופציונלי"

    `pybabel extract` אוסף הערות למתרגמים רק כאשר מעבירים
    `-c "Translators:"`, בדיוק כפי שהוא נוהג בקריאות gettext רגילות.
...
## עמיד כברירת מחדל { #robust-by-default }

קובץ פגום אחד אינו מסיים את הריצה:
...
הגדירו `strict = true` באפשרויות המיפוי כדי להפוך כל אחד מהמקרים האלה
לכשל קשיח במקום זאת, וזה מה שתרצו ב-CI.
...
דגל. התנהגותן היא עניינן; שני הכלים שלהלן הם אלה שאומתו כאן.
...
שתי הבדיקות אינן כפל מיותר. הבודק המצורף הוא הצד הקפדני יותר בלפחות
שני מקומות:
