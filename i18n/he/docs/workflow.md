...
הסיכום של כל זה הוא שש בדיקות, ולכן הנה הן תחילה; כל סעיף שלהלן מקים
אחת מהן.

- `pybabel update --check` עובר — שום הודעה לא השתנתה בלי שהקטלוגים שמעו
  על כך.
- `pybabel compile` חוסם את הבנייה על סמך קוד היציאה שלו.
- רשומות `fuzzy` שנותרו הן מכוונות — כל אחת מהן מרונדרת כטקסט המקור עד
  שמתרגם מאשר אותה.
- מערך הבדיקות מרנדר כל שפה נשלחת פעם אחת עם `strict=True`.
- תוצר סביבת הייצור מכיל קובצי `.mo` ולא מכיל את Babel.
- הלוגר `gettext_tstrings` מנותב אל הניטור.
...
!!! bug "Babel 2.18.0: `--check` אינו יכול לחסום קטלוג שמשתמש בהקשרים"
    ...
    וזה מה ש[הבנייה של האתר הזה עצמו](index.md) עושה. הסיבה
    [מתועדת בעמוד המלכודות](pitfalls.md#your-tools-have-bugs-too).
