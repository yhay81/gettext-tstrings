...
היעד בדוגמה של עמוד זה, אבל שום דבר לא תלוי בבחירה הזו. כדי להשתמש בשפה
אחרת, החליפו את `ja` בשלב 4 — קוד השפה הזה הוא הדבר היחיד שמציין אותה.
...
מתרגמים עובדים בדרך כלל מתוך קטלוגים ולא מתוך קוד מקור, ולכן קובץ קטן
שנקרא **קטלוג** הוא שנודד ביניכם לבינם. הצעד הראשון לקראתו הוא איסוף כל
הודעה מסומנת מתוך הקוד.
...
הסתייגות אחת שכדאי להכיר כבר עכשיו: הפקודה מדווחת על השגיאה ויוצאת עם
קוד שונה מאפס, אבל כותבת את קובץ ה-`.mo` בכל מקרה. בפרויקט אמיתי ה-CI
הוא שחייב לעצור בגלל קוד היציאה הזה — [בסביבת ייצור](workflow.md#what-ci-gates)
מגדיר זאת.

## 5. הרצה { #5-run-it }

שלבים 2–4 השתמשו ב-`tr()`, שמחפש קטלוג ואינו מוצא אף אחד. עכשיו כשקיים
אחד, טענו אותו וקשרו אותו פעם אחת: `Translator` מחזיק קטלוג כדי שאתרי
הקריאה לא יצטרכו לציין אותו, ו-`_` הוא השם המקובל ב-gettext לתוצאה.
...
1. הספרייה הסטנדרטית טוענת את קובץ ה-`.mo` המקומפל, ו-Translator קושר
   ...
   את זה" — קצר, כי הוא מופיע על כל מחרוזת הפונה למשתמש. הוא מבצע את
   אותו תרגום כמו `tr`, קשור לקטלוג אחד.
...
- [מעבר מקיים](migration.md) — אם לפרויקט שבו אתם באמת רוצים לעשות זאת
  כבר יש קטלוגי gettext.
- [למתרגמים](translators.md) — העמוד היחיד שכדאי למסור למי שממלא בפועל
  את שורות ה-`msgstr` האלה.
