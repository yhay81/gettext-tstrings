---
description: "同一則可翻譯訊息分別以 %-format、.format()、flufl.i18n 的 $-string 與 t-string 寫成，並比較各自如何繫結值、又如何面對壞掉的目錄。"
---

# 為什麼選擇 t-string

把一個值放進可翻譯訊息的四種做法，以同一句話互相比較。先講結論：

- 用 **%-format**，譯者少刪一個字母，就會在正式環境變成程式當掉。
- 用 **str.format**，翻譯可以讀取你的程式碼傳進去的物件屬性——包括機密資料。
- 用 **$-string**（flufl.i18n），值是從呼叫端函式的變數裡隱含取得的，而帶點號的
  佔位符同樣能碰到屬性。
- 用 **t-string**，格式設定留在你的程式碼裡，翻譯會在執行階段被檢查，而壞掉的
  目錄會回退為原文，不會讓程式當掉。

本頁其餘篇幅就是佐證，一種做法一種做法地看。
...
在 PO 編輯器裡改一個字元，就成了正式環境的 traceback。GNU
`msgfmt --check-format` 確實抓得到它，但僅限於被標上 `python-format` 的訊息，而且
還要目錄在送進你的應用程式的路上真的經過 msgfmt 才行。
...
被拒絕不等於當掉：預設情況下本函式庫會記錄一則警告並渲染原文，所以壞掉的目錄
...
`:,.2f` 從不會抵達目錄，所以沒有任何翻譯能改動它，也沒有任何譯者需要看到它。
...
| 翻譯*漏掉*一個佔位符——會渲染出什麼？ | 值無聲無息地消失 | 值無聲無息地消失 | 值無聲無息地消失 | 原文，外加一則警告（[預設](guide.md#what-happens-when-a-catalog-is-wrong)） |
| 翻譯*多加*一個未知佔位符——會渲染出什麼？ | 一個例外 | 一個例外 | 佔位符以文字形式留著可見 | 原文，外加一則警告（[預設](guide.md#what-happens-when-a-catalog-is-wrong)） |
...
近似 f-string 的語法以及明確的值繫結。`$`-string 本身已經提供了一種簡潔的替代方案，
只是繫結方式與失敗模型不同。`flufl.i18n` 是一個成熟的套件，在 Python 3.10 以上都能
跑；`gettext-tstrings` 目前還是 alpha，而且因為 t-string 是新語法，它需要 Python
3.14 以上。

另一項代價就是那條限制本身：插值必須是一個單純的名稱。
...
那是一項實實在在的約束。它與來源端的值繫結、執行階段的佔位符檢查合在一起，阻止了
目錄字串去求值運算式，也讓佔位符名稱保持有意義。
