...
# 用 Python 的 t-string<br>翻譯完整的訊息
...
[開始五分鐘的教學 :material-arrow-right:](tutorial.md){ .md-button .md-button--primary }
...
本函式庫會在入口檢查這一點——也就是目錄編譯的時候——渲染時再檢查一次；這正是「在
審查時就發現的錯誤」和「由使用者發現的錯誤」之間的差別。
