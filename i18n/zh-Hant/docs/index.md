---
description: "透過 gettext 與 Babel 翻譯完整的 t-string 訊息，並把格式設定留在目錄之外。"
...
為 Python 3.14+ 的 t-string 提供安全的 gettext 與 Babel 整合——值留在原處，
目錄看到的則是完整的訊息：
...
[為什麼選擇 t-string](comparison.md){ .md-button }
...
目錄收到的是完整的句子 `Hello {name}`。翻譯可以調換或重複 `{name}` 的位置，但
不能把它刪掉、憑空造一個出來，也不能自行加上格式設定——本函式庫會檢查這些情況，
而且目錄有問題時會回退為原文，而不是讓程式當掉。

!!! note "第一次接觸 gettext？四句話講完整套流程"

    **gettext** 是軟體翻譯的標準做法，在 Python 內外都通用。你的程式碼標記出
    可翻譯的字串；*擷取器*把它們收集到樣板檔（`.pot`）；譯者——通常不是
    程式設計師——為每種語言填寫一份目錄檔（`.po`），再編譯成二進位的 `.mo`，
    由應用程式在執行階段載入。翻譯函式的慣用名稱是 `_`，所以
    `_(t"Hello {name}")` 讀起來就是「把這句話翻譯出來」。**[教學](tutorial.md)**
...
## 它做出的取捨 { #the-choice-it-makes }

- 永遠翻譯完整的訊息，不翻譯句子片段。
- 只接受 `{name}` 這類單純的變數名稱。
- 讓 `!r` 和 `:.2f` 留在應用程式手上，不進入目錄。
- 允許譯者調換與重複已知的佔位符——但不能存取屬性，也不能加上格式化行為。
...
會來到這裡的讀者分成三種：正在翻譯自己第一支程式的人、正要把翻譯接進真實專案
的人，以及想確切知道這套機制為何長成這樣的人。每一種都有自己的路線。

**學會它**——不預設你有 gettext 經驗：

<div class="grid cards" markdown>

- **[教學](tutorial.md)** — 從這裡開始：五個步驟，從一個空資料夾走到一份跑得
  起來的日文翻譯，每道指令都附上輸出。
